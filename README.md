...
이 저장소가 업데이트돼도 내 워커가 저절로 바뀌지는 않습니다. 설치했던 방법에 따라:

## 붙여넣기(방법 B)로 설치한 경우 — 2분

1. 새 탭에서 최신 코드를 열어 **전체 선택(Ctrl+A) → 복사**:
   **https://raw.githubusercontent.com/azit4u/tamsaek-worker/main/worker.js**
2. 클라우드플레어 대시보드 → 내 워커(tamsaek-worker) → **Edit code**
3. 편집기의 기존 코드를 전체 선택해서 지우고 → 붙여넣기 → **Deploy**

워커 주소·비밀키·AI 바인딩은 코드와 따로 저장되므로 **그대로 유지됩니다** — 워드프레스 설정을 다시 만질 필요 없습니다.

## 원클릭 버튼(방법 A)으로 설치한 경우 — 2분

배포 버튼을 **다시 누르지 마세요** (복사본 저장소가 하나 더 생깁니다). 대신:

1. 배포 때 내 깃허브에 생긴 **내 복사본 저장소**(tamsaek-worker)를 엽니다
2. `worker.js` 파일 클릭 → 연필 아이콘(Edit) → 위의 raw 주소에서 복사한 최신 코드로 전체 교체
3. **Commit changes** → 커밋되면 클라우드플레어가 자동으로 재배포합니다

## 업데이트가 됐는지 확인

브라우저에서 내 워커 주소(`https://tamsaek-worker.아이디.workers.dev`)를 열었을 때 안내 페이지가 뜨고, 경로 목록에 `/api/research`가 보이면 최신 계열 코드입니다.
...
**Q. 검색 결과가 갑자기 줄었어요.**
검색 사이트가 화면 구조를 바꾸면 생길 수 있습니다. 이 저장소가 업데이트되면 위의 **「업데이트 방법」** 대로 새 코드를 반영하면 됩니다.
